...
app = FastAPI(title="简化优化版 - 实时技术新闻聚合器", version="2.1.0")
...
class SimpleOptimizedNewsProcessor:
    def __init__(self):
...
# 全局新闻处理器
news_processor = SimpleOptimizedNewsProcessor()
...
async def generate_optimized_news_stream(news_per_second: int = 1000, duration: int = 30):
    """生成优化的新闻流"""
    try:
        print(f"📡 启动简化优化新闻生成器: {news_per_second}条/秒，持续{duration}秒")
...
async def get():
    ...
    <head>
        ...
        <style>
            ...
            .optimization { background: #27ae60; color: white; padding: 10px; border-radius: 5px; margin: 5px 0; }
            .performance { background: #e74c3c; color: white; padding: 10px; border-radius: 5px; margin: 5px 0; }
    ...
    <body>
        <div class="container">
            ...
            <div class="optimization" id="optimization">
                ...
                <p>广播批次: <span id="batch-count">0</span></p>
            </div>
            ...
            ws.onmessage = function(event) {
                ...
                if (data.type === 'statistics') {
                    ...
                    if (data.data.broadcast_stats) {
                        ...
                        batchCount.textContent = data.data.broadcast_stats.batch_count;
                    }
